...
export class TranslatedImageCache {
  ...
  /**
   * Set a cached URL directly (for pre-rendered images from backend).
   */
  set(hash: string, translatedUrl: string): void {
    this.memoryCache.set(hash, {
      originalSrc: '',
      translatedBlob: new Blob(), // Empty blob for direct URLs
      translatedUrl,
      regions: [],
      width: 0,
      height: 0,
